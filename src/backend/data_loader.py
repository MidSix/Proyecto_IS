...
class DataModule:
    ...
    def __init__(self):
        ...
        self.error_message = None #To store error messages and show them in the GUI, this variable will store a string with the current error message and be none when the last operation was successful
    ...
    def add_file_path(self, file_path: str, file_name: str) -> bool:
        ...
        if not os.path.isfile(file_path) or file_type not in ['sqlite', 'db', 'csv', 'xlsx', 'xls']:
            self.error_message = "File does not exist or is incompatible. Please provide a valid file path."
            return False
    ...
    def set_current_file(self, file_name: str) -> bool:#This is for the file selector in the GUI
        if file_name not in self.file_lists:
            self.error_message = "File name not found. Please provide a valid file name."
            return False
        self.error_message = None
    ...
    def load_data(self) -> bool: #returns True if data load succesfully, false otherwise.
        try:
            if self.current_file_type in ['sqlite', 'db']:
                ...
                if not tables:#I think it's ok to raise error here because it's an exceptional case
                    self.error_message = "No table found in the sqlite database."
    ...
    def main(self, file_path: str):#To simplify things and encapsulate this module. GUI will
        #call this function that handles all the internal logic and returns the dataframe associated
        #to the given path.
        """Wrapper that adds, selects and loads a file. Returns (DataFrame or None, error_message or None)."""
        self.error_message = None
        file_name = os.path.basename(file_path) if file_path else None

        ok = self.add_file_path(file_path, file_name)
        if not ok: #I simply love this if statment XD
        ...
        return self.current_dataframe, None #Basically if it reaches this point everything went ok so return the dataframe and no error message
    ...
    def direct_data_load(self, data:pd.DataFrame)->bool:
        ...
        if not isinstance(data, pd.DataFrame):
            self.error_message = "Provided data is not a pandas DataFrame."
            return False
        self.current_dataframe = data
        return True
    #------ Functions added for linear regression testing purposes ------

# The next class was moved from another branch,
# just moved, so it hasn't any important or 
# valuable change on it.
