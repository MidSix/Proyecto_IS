import sqlite3
import pandas as pd
import os # To handle file paths and extensions
#that may vary by OS(linux, windows, macOS)

#This data module must support SQlite, CSV, and Excel formats
#I decided to use pandas for the general dataframe handling,
#because it provides a matrix-like structure and easy data manipulation
class DataModule:
    ...
    def set_current_file(self, file_name: str) -> bool:#This is for the file selector in the GUI
        try:
            if file_name not in self.file_lists:
                raise ValueError("File name not found in the path list.")
        except Exception as e:
            error_message = f"Error setting current file: {e}"
